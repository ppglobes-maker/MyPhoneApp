    ...
    try {
      ...
      if (Platform.OS === 'web') {
        if (!pickedAsset.base64) {
          Alert.alert('Error', 'Could not read selected image for web.');
          return;
        }
        const mimeType = pickedAsset.mimeType || 'image/jpeg';
        savedUri = `data:${mimeType};base64,${pickedAsset.base64}`;
  ...
  return (
    <View style={styles.screen}>
      <ImageBackground
        ...
        style={[styles.image, Platform.OS === 'web' ? styles.imageWebAdjust : null]}
        fadeDuration={0}
        resizeMode="cover">
        {!showLoginScreen ? (
          <>
